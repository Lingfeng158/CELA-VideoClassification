    ...
    for category in labelLst:
        ...
        for subject in subjects:
            ...
            for clip in clips:
                ...
                frameData.sort()
                ##################################################################
                #for data_length>frameToUse, randomly select a segment of frames #
                ##################################################################
                tag=True
                while(tag):
                    #select a part of frames if data length is large
                    if(data_length>framesToUse):
                        frameDataToUse=frameData[:framesToUse]
                        frameData=frameData[framesToUse:]
                        data_length=len(frameData)
                    #update available framedata
                        
                    #else select all frames
                    else:
                        tag=False
                        frameDataToUse=frameData
